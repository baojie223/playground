...
class RangeList {
   ...
   * @returns {boolean}
   */
  #isValidRange(x: unknown): x is Range {
    if (
      !(
        Array.isArray(x) &&
        x.length === 2 &&
   ...
   */
  #add(ranges: Range[], newRange: Range) {
    const n = ranges.length
    const res: Range[] = []
    let i = 0

    // push ranges smaller than newRange into result
    ...
    // push ranges bigger than newRange into result
    while (i < n) {
      res.push(ranges[i])
      i++
    }
   ...
   */
  #remove(ranges: Range[], removedRange: Range) {
    const res: Range[] = []

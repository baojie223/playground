...
function updateDom(dom, prevProps, nextProps) {
  Object.keys(prevProps).filter(isEvent).filter(key => nextProps[key] !== prevProps[key] || isNew(prevProps, nextProps)(key)).forEach(name => {
  ...
  Object.keys(prevProps).filter(isProperty).filter(isNew(prevProps, nextProps)).forEach(name => {
...
window.onload = () => {
  function Counter() {
    ...
    return Vue.createElement("h1", {
      onClick: () => setState(c => c + 1)
    }, "Count: ", state);
  }

  const element = Vue.createElement(Counter, null);
